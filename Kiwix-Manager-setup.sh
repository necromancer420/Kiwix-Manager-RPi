...
(if [ ! -e "$HOME/Kiwix-Manager" ]; then
mkdir "$HOME/Kiwix-Manager"
sleep 1
echo -e "${LGREEN}Directory Created: $HOME/Kiwix-Manager/"
else
echo -e "${LRED}Directory Exists: $HOME/Kiwix-Manager/"
echo -e "${LGREEN}Refreshing Files"
fi)
mv "processHelper.pim" "$HOME/Kiwix-Manager/Kiwix-Manager.sh"
mv "README.MD" "$HOME/Kiwix-Manager/README.MD"
mv "wiki.png" "$HOME/Kiwix-Manager/wiki.png"
sudo cp "$HOME/Kiwix-Manager/wiki.png" /usr/share/pixmaps/wiki.png
sleep 1
echo -e "${LGREEN}Files Copied"
chmod u+x "$HOME/Kiwix-Manager/Kiwix-Manager.sh"
echo -e "Script made Executable"
printf " -Once zims download the 1st time, running the download option will only upate that zim if needed, otherwise it is skipped.\n\n -Unchanged zims will show no progress bar on-screen during the download script. Be calm, It works!\n\n -The library_zim.xml IS NOT overwritten each time you run this script, it is only added to, unless you choose option #5 to delete entries.\n\n -If you see a difference between number of zims and number of XML entries, it is usually due to that zim not having information on kiwix.org for their xml handler(kiwix-manage) to use." > "$HOME/Kiwix-Manager/KIWIX-README"
rm -rf $(basename $0) 
sleep 1
cd "$HOME/Kiwix-Manager"
rm -rf "${DIRECTORY}"
echo "Creating menu button"
echo "[Desktop Entry]
Name=Kiwix Manager
Comment=Install, Uninstall, Add/remove Zims, Manage Library XML
Exec="$HOME/Kiwix-Manager/Kiwix-Manager.sh"
Icon="$HOME/Kiwix-Manager/wiki.png"
Terminal=true
Type=Application
...
cd "$HOME/Kiwix-Manager"
echo -e ""
echo -e "${LGREEN}You can now run the program from $HOME/Kiwix-Manager/Kiwix-Manager.sh"
			echo -e ""
			echo -e "${LCYAN}Press any key to continue..."
			read -n 1 -s
bash "$HOME/Kiwix-Manager/Kiwix-Manager.sh"
