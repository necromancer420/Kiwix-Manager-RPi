...
#Make Help Files
printf " -Once zims download the first time, running the download option will only update that zim if needed, otherwise it is skipped. This means you can use the download option anytime just to ensure your zim file and associated library entry are up to date. (Works great with often-updated zims like WikiNews.)\n\n -Unchanged zims will show no progress bar on-screen during the download script. Be calm, It works." > "${NEWDIR}/Download.hlp"
